...
if(instance_exists(obj_playerB)){
	if(place_meeting(x,y+acc,obj_playerB)&&y+acc<=obj_playerB.y&&!obj_playerB.isBubbled){
		if(falling||isWinged){
			obj_playerB.isWinged=false;
			var temp_player_x=obj_playerB.x;
			var temp_player_y=obj_playerB.y;
			instance_create_layer(temp_player_x,temp_player_y,"Instances",obj_star);
			audio_play_sound(snd_hit,0,false);
			vspeed=clamp(vspeed*-0.98,ySpdMax,ySpdMin);
			falling=false;
			obj_playerB.vspeed=30;
			obj_playerB.control=false;
			global.playerAScore+=1;
		}
	}
}
if(falling&&vspeed>=5){
	id.sprite_index=spr_playerA_down;
}
else if(falling&&vspeed<5){
	id.sprite_index=spr_playerA_still;
}
else{
	id.sprite_index=spr_playerA_up;
}
if(isBubbled){
	if(!isBubbledAlarm){
